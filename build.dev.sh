...
docker build --build-arg GIT_COMMIT=$(git rev-parse HEAD) -t $REGISTRY/server -f infra/docker/Dockerfile.server .
docker build --build-arg GIT_COMMIT=$(git rev-parse HEAD) -t $REGISTRY/client -f infra/docker/Dockerfile.client .
